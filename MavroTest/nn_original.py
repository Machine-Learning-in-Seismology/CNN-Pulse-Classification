...
def folding(y, o, n_folds):
    ...
    for i in range(n_folds):
        ...
        mask2 = np.ones(synthetics.shape, dtype=bool)
        mask2[(i * step): (i * step + train_neg_len)] = False
        testing_syn = synthetics[~mask2]
        training_syn = synthetics[mask2]
        testing = np.append(testing,testing_syn)
        print('Training Synthetics',training_syn.shape)
        print('Testing Synthetics',testing_syn.shape)
	#np.random.shuffle(synthetics)
        #training = np.append(training, synthetics)
        np.random.shuffle(training_syn)
        training = np.append(training,training_syn)
...
def do_learning(x, y, syn):
    ...
    #    intv2 = integrate.cumtrapz(data**2, dx=1/20)
    #    x[i,:] = np.append(intv2[0],intv2)
    #tmp = integrate.cumtrapz(x[0,:]**2, dx=1/20)
    #plt.plot(tmp)
    #plt.show()
    y = np.append(ny, no)
    ...
    for n in networks:
        fold = 0
        for train_index, test_index in folding(ny, no, 5):
